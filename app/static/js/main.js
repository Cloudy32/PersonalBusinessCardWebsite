const translations = {
  ru: {
    ...
    aboutOrigin:
      "Я родился на севере России, в Ханты-Мансийском автономном округе. Сейчас мне 23 года, я живу в более тёплой части России — в городе Брянск.",
    aboutDeveloperPath:
      "Уже <strong>3,5 года</strong> я развиваюсь как <strong>Python Backend Developer</strong>. Сейчас я двигаюсь дальше не только как человек, который пишет код, а как личностный разработчик: создаю свои проекты, развиваю портфолио, делюсь опытом и выстраиваю вокруг себя профессиональный путь.",
    aboutStart:
      "<strong>Мой путь в IT начался около 5 лет назад</strong>. Тогда я работал в небольшой компании и заполнял таблицы. В какой-то момент мне захотелось упростить рутинную работу. Именно тогда я заинтересовался автоматизацией и постепенно пришёл к программированию. Со временем я понял, разработка — это не просто “писать строчки кода” или нажимать кнопки. Это возможность <strong>создавать, упрощать и строить новые</strong> цифровые решения вокруг себя 🚀",
    aboutMeaning:
      "",
    aboutStackIntro:
      "За это время я поработал с разными задачами и технологиями. Изначально мой стек был довольно простым: Python, ООП и Pandas. Сейчас он выглядит так:",
    aboutStack:
    ...
    aboutFrontend: "Также я развиваюсь во <strong>frontend-разработке</strong> — этот сайт является одним из примеров такого опыта.",
    aboutSearch:
      "<strong>Сейчас я активно ищу работу на позицию Middle Python Backend Developer</strong> и открыт к интересным предложениям. Мои контакты для связи указаны выше 📩",
    aboutOutro: "",
    photoOne: "Мое фото",
    photoTwo: "Мое фото №2",
